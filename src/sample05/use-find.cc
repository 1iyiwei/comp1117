...
int main(){
	...
	cout << "Enter a URL: ";   // 
	string url;                //     
	cin >> url;                //      http://www.cs.hku.hk/~c1117c

	int colon_pos = url.find(":");    
	//                       4
	if (colon_pos == string::npos){
		cout << "Error: invalid URL!  It should be in the format <protocol>:..." 
		<< endl;
		return 0;   // Bye Bye!
	}

	// there is a colon  
	string protocol = url.substr(0, colon_pos);
	//                         http                  
	string temp = url.substr(colon_pos + 1);      // chop off the protocol part
	//  //www.cs.hku.hk/~c1117c                  
	if (temp.find("//") != 0){
		cout << "Error: invalid URL! The format should be <protocol>://..."
		<< endl;
		return 0;
	}
